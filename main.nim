import std/[strformat, strutils, math, os]
...
import src/[prefsmodal, editor, utils, icons]
...
proc updatePrefsCache(app: var App) = app.prefsCache = app.prefs.content
...
proc switchTheme(app: var App, themeIndex: int) = 
  ...
  app.splitterSize1.a = 0f
  app.splitterSize2.b = 0f
...
proc drawThemesList(app: var App) = 
  ...
  if igBeginListBox("##themes", igVec2(app.splitterSize1.a, igGetContentRegionAvail().y - igGetFrameHeight() - style.windowPadding.y)):
...
proc drawEditView(app: var App) = 
  ...
  if app.prevAvail != igVec2(0, 0) and app.prevAvail != avail:
    if app.editing:
      (app.splitterSize1, app.splitterSize2) = (
        ((app.splitterSize1.a / app.prevAvail.x) * avail.x, 0f), 
        ((app.splitterSize2.a / app.prevAvail.x) * avail.x, (app.splitterSize2.b / app.prevAvail.x) * avail.x)
      )
    else:
      (app.splitterSize1, app.splitterSize2) = (
        ((app.splitterSize1.a / app.prevAvail.x) * avail.x, 0f), 
        ((app.splitterSize2.a / app.prevAvail.x) * avail.x, 0f)
      )

  app.prevAvail = avail

  # First time or when switch editing
  if app.editing and app.splitterSize2.b == 0f:
    (app.splitterSize1, app.splitterSize2) = ((avail.x * 0.2f, 0f), (avail.x * 0.4f, avail.x * 0.4f))
  elif app.splitterSize1.a == 0f:
    (app.splitterSize1, app.splitterSize2) = ((avail.x * 0.5f, 0f), (avail.x * 0.5f, 0f))

  igSplitter(true, 8, app.splitterSize1.a.addr, app.splitterSize2.a.addr, style.windowMinSize.x, style.windowMinSize.x, avail.y)
  # List
  if igBeginChild("##editViewThemes", igVec2(app.splitterSize1.a, avail.y), flags = makeFlags(AlwaysUseWindowPadding)):
    app.drawThemesList()
  igEndChild(); igSameLine()

  # Second Splitter
  if igBeginChild("##editViewSplitter2", igVec2(app.splitterSize2.a + app.splitterSize2.b, avail.y)):
    igSplitter(true, 8, app.splitterSize2.a.addr, app.splitterSize2.b.addr, style.windowMinSize.x, if app.editing: style.windowMinSize.x else: 0, avail.y)
    # Preview
    if igBeginChild("##editViewPreviewer", igVec2(app.splitterSize2.a, avail.y), flags = makeFlags(AlwaysUseWindowPadding)) and app.currentTheme >= 0:
    ...
    if app.editing:
      ...
      if igBeginChild("##editViewEditor", igVec2(app.splitterSize2.b, avail.y), flags = makeFlags(AlwaysUseWindowPadding, HorizontalScrollbar)):
...
proc initApp(config: PObjectType): App = 
  result = App(
    ...
    splitterSize1: (0f, 0f), splitterSize2: (0f, 0f)
  )
  result.initPrefs()
