...
proc hasValid(theme: TomlTableRef, name: string, key: string, kind: TomlKind, optional = false): bool = 
  ...
  elif (not optional or (optional and key in theme)) and kind in {TomlKind.Array, TomlKind.Tables, TomlKind.Table, TomlKind.String} and theme[key].len == 0:
...
proc check(): seq[tuple[idx: int, name: string]] = 
  assert fileExists(getCurrentDir() / themesPath), &"E: Could not find {themesPath} in the current directory"
  ...
  try:
    ...
    assert data["themes"].kind == TomlKind.Tables, "E: The themes key is not an array of tables"
  ...
  for e, theme in themes:
    ...
    if not theme.hasValid(name, "author", TomlKind.String): valid = false
    if not theme.hasValid(name, "description", TomlKind.String): valid = false
    if not theme.hasValid(name, "tags", TomlKind.Array): valid = false
    if not theme.hasValid(name, "date", TomlKind.DateTime): valid = false
    if not theme.hasValid(name, "forkedFrom", TomlKind.String, optional = true): valid = false
